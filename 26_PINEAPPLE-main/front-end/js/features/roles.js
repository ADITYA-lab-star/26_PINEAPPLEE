...
function getAdminProjectCount(userRecord, seed = {}) {
  ...
  if (!projects) {
    const fallback = Number(seed.projects || 0);
...
function getAllAdminUsers() {
  ...
  const result = Object.entries(usersStore || {}).map(([email, userRecord]) => {
    ...
    const projects = getAdminProjectCount(userRecord, seed);
    ...
    return {
      ...
      xp: Number(profile.xp ?? seed.xp ?? 0),
      rep: Number(profile.rep ?? seed.rep ?? 0),
      projects,
